...
def get_feed_entries(helper, name, start, stats):
    ...
    feed_server = helper.get_arg('feed_server')
    feed_output_name = helper.get_arg('feed_output_name')
    feed_url = "{0}/feeds/{1}".format(feed_server,feed_output_name)
    feed_username= helper.get_arg('feed_username')
    feed_password = helper.get_arg('feed_password')
    feed_headers = {}
    # If auth is specified, add it as a header.
    if feed_password is not None:
        helper.log_info("Feed credentials: feed_username={0} feed_password=********")
        auth = '{0}:{1}'.format(feed_username, feed_password).encode('ascii')
        auth = base64.b64encode(auth)
        auth = auth.decode('utf-8')
        feed_headers['Authorization'] = 'Basic {0}'.format(auth)
    else:
        helper.log_error("Failed to find credentials for feed.")

    # Pull events as json.
    helper.log_info("Pulling indicators from feed: {0}".format(feed_url))
    resp = helper.send_http_request(
        url=feed_url,
        method='GET',
